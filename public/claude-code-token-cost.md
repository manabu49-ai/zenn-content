...
:::note warn
**追記（2026-09-04）― この記事の書き方に不足がありました**

本文はトークンの**量**を測っていますが、**量と金額は別です。**
キャッシュ読出は入力の 1/10、出力の 1/50 の単価で課金されるため、
**トークン数をそのままコストの代理として読むと過大評価になります。**

自分のログ 25,574 ターンで測り直しました。
...
**キャッシュ読出は出力の344倍（トークン量）ですが、金額比では6.88倍です。**

**結論の向き（読出が金額でも最大）は変わりません**が、
「23万トークン」という数字を**そのまま費用の大きさとして読まないでください。**

詳細と、別の書き手が同じ測定で**違う結論**に至った件は、こちらに書きました。
（連載7参照）
:::
